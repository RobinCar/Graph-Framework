...
public class AdjacencyMatrixDirectedGraph extends AbstractMatrixGraph<DirectedNode> implements IDirectedGraph {
	...
	public IDirectedGraph<DirectedNode> computeInverse() {
		int[][] matrixRet = new int[this.order][this.order];
		for (int i = 0; i < this.order; i++) {
			for (int j = 0; j < this.order; j++) {
				matrixRet[i][j] = this.matrix[j][i];
			}
		}
		return new AdjacencyMatrixDirectedGraph(matrixRet);
	}
	...
	public static void main(String[] args) {
		...
		am.addArc(new DirectedNode(2), new DirectedNode(3));
		am.addArc(new DirectedNode(2), new DirectedNode(3));
		am.addArc(new DirectedNode(2), new DirectedNode(3));
		System.out.println();
		System.out.println(am);
		am.removeArc(new DirectedNode(2), new DirectedNode(3));
		System.out.println();
		System.out.println(am);
		System.out.println();
		System.out.println(am.isArc(new DirectedNode(2), new DirectedNode(3)));
	}
}
